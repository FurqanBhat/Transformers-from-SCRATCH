...
class InputEmbeddings(nn.Module):
    ...
    def __init__(self, d_model, vocab_size):  #d_model=dimension of embedding vectors
        super().__init__()
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x):
        mean=x.mean(dim=-1, keepdim=True)  #calculating mean of last dim
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout:nn.Dropout):
        ...
        attention_scores=(query @ key.transpose(-2, -1))/math.sqrt(d_k) #transpose last 2 dims, (batch, h, seq_len, seq_len)
        if mask is not None:
            attention_scores.masked_fill(mask==0, -1e9)
        attention_scores=attention_scores.softmax(dim=-1) #along last dim=rows, (batch, h, seq_len, seq_len)
        if dropout is not None:
            attention_scores=dropout(attention_scores)
        
        output=(attention_scores @ value) #(batch, h, seq_len, d_k)
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, subLayer):
        ...
        #Not in the originla paper but used recently and works better sometimes
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        #x=decoder input, src_mask=mask applied to encoder, tgt_mask=mask applied to decoder
